  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
          ...
          <div className="px-4 pt-3 pb-3.5">
            <div className="flex items-center justify-between mb-2.5">
              ...
              <span className="text-[10px] font-medium text-slate-400">{100 - progress}% left</span>
            </div>
            <div className="flex items-baseline gap-0.5 mb-2">
              <span className="text-2xl font-bold text-slate-900 leading-none">{progress}</span>
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
          ...
          <div className="px-4 pt-3 pb-3.5">
            ...
            <div className="flex items-baseline gap-0.5 mb-2">
              <span className="text-2xl font-bold text-slate-900 leading-none">{project.completed_task_count}</span>
              <span className="text-sm text-slate-300 font-semibold">/{project.task_count}</span>
              <span className="text-[10px] text-slate-400 ml-1">done</span>
            </div>
          ...
          <div className="px-4 pt-3 pb-3.5">
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-1.5">
                <div>
                  <p className="text-[9px] text-slate-400 uppercase tracking-wide leading-none mb-0.5">Start</p>
                  <p className="text-xs font-semibold text-slate-800 leading-none">
                    {project.start_date ? formatDate(project.start_date) : 'Not set'}
                  </p>
                </div>
                <div>
                  <p className="text-[9px] text-slate-400 uppercase tracking-wide leading-none mb-0.5">Deadline</p>
                  <p className="text-xs font-semibold text-slate-800 leading-none">
          ...
          <div className="px-4 pt-3 pb-3.5">
            <div className="flex items-center justify-between mb-2.5">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest">Team</span>
              </div>
              {project.members && (
            ...
            {project.lead && (
              <div className="flex items-center gap-1 mt-1.5">
                ...
                <p className="text-[10px] text-slate-400 truncate">
                  Lead: <span className="text-slate-700 font-semibold">{project.lead.full_name}</span>
                </p>
              </div>
            )}
          </div>
              ...
              {(Object.keys(tasksByStatus) as (keyof typeof tasksByStatus)[]).map((status) => {
                ...
                return (
                  <div key={status} className="flex items-center gap-3 group">
                    ...
                    <span className="text-sm font-bold text-slate-700 w-5 text-right tabular-nums">{count}</span>
            ...
            {issues.length > 0 ? (
              <div className="space-y-1">
                {issues.slice(0, 4).map((issue, idx) => (
                  ...
                  >
                    <span className="text-[11px] font-mono text-slate-300 w-4 shrink-0 group-hover:text-slate-400 transition-colors">
                      {String(idx + 1).padStart(2, '0')}
                    </span>
                    <Badge className={ISSUE_TYPE_COLORS[issue.type]} size="sm">
                      {ISSUE_TYPE_LABELS[issue.type]}
                    </Badge>
                    <p className="text-sm text-slate-700 flex-1 truncate font-medium">{issue.title}</p>
      ...
      {activeTab === 'tasks' && (
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <h3 className="font-semibold text-slate-900">Tasks ({tasks.length})</h3>
          ...
          <div className="divide-y divide-slate-50">
            {tasks.map((task) => (
              <div key={task.id} className="flex items-center gap-4 px-6 py-4 hover:bg-slate-50/50 transition-colors">
            ...
            {tasks.length === 0 && (
              <p className="text-sm text-slate-400 text-center py-5">No tasks yet</p>
            )}
          </div>
        </div>
      )}
      ...
      {activeTab === 'issues' && (
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100">
            <h3 className="font-semibold text-slate-900">Issues ({issues.length})</h3>
          </div>
          <div className="divide-y divide-slate-50">
            {issues.map((issue) => (
              <div key={issue.id} className="flex items-center gap-4 px-6 py-4 hover:bg-slate-50/50 transition-colors">
                <Badge className={ISSUE_TYPE_COLORS[issue.type]} size="sm">{ISSUE_TYPE_LABELS[issue.type]}</Badge>
                <p className="text-sm font-medium text-slate-900 flex-1">{issue.title}</p>
                ...
                {issue.reporter && (
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-xs text-slate-500 hidden sm:block">{issue.reporter.full_name}</span>
                  </div>
                )}
              </div>
            ))}
            {issues.length === 0 && (
              <p className="text-sm text-slate-400 text-center py-5">No issues reported</p>
            )}
          </div>
        </div>
      )}
      ...
      {activeTab === 'team' && (
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100">
            <h3 className="font-semibold text-slate-900">Team Members ({project.members?.length || 0})</h3>
          </div>
          <div className="divide-y divide-slate-50">
